...
/** KRO-198 — `justify-self` del CHIP en su celda (modo `chipWidth:'content'`): el chip
 *  se AJUSTA AL CONTENIDO y `align` lo POSICIONA (izq/centro/der). Literales p/ Tailwind. */
const CHIP_JUSTIFY_SELF: Record<NonNullable<SlotAppearance['align']>, string> = {
  left:   'justify-self-start',
  center: 'justify-self-center',
  right:  'justify-self-end',
};
/** Clase justify-self del chip según su align — para colocar el chip content-fit en su
 *  celda. '' si no hay align (cae a justify-self-start por defecto del grid). */
...
/** KRO-198 — `justify-content` del CHIP-PASTILLA (modo `chipWidth:'fill'`): el chip llena
 *  la celda y `align` coloca el TEXTO dentro de la pastilla (es `inline-flex`, así que el
 *  text-align no lo mueve → se usa justify-content). Literales p/ Tailwind. */
const CHIP_JUSTIFY_CONTENT: Record<NonNullable<SlotAppearance['align']>, string> = {
  left:   'justify-start',
  center: 'justify-center',
  right:  'justify-end',
};
export function chipJustifyContent(align: SlotAppearance['align'] | undefined): string | undefined {
  return align ? CHIP_JUSTIFY_CONTENT[align] : undefined;
}
...
export function ComposableSlot({
  ...
  const styleFor = (key?: string) => {
    ...
    return {
      ...
      display: ap?.display,
      // KRO-198 — align + ancho del chip → colocación en la rejilla (ver rama chips).
      align: ap?.align,
      chipWidth: ap?.chipWidth,
    };
  };
      ...
      return (
        ...
        >
          {entries.map((e, i) => {
            ...
            // KRO-198 — ANCHO del chip (solo en rejilla): `chipWidth:'content'` → el chip se
            // AJUSTA AL CONTENIDO y `align` lo POSICIONA (justify-self). `'fill'` (default) →
            // el chip estira (llena la celda) y `align` coloca el TEXTO dentro (justify-content
            // en la pastilla / text-align en el texto plano). Dos ejes independientes.
            const isContent = s.chipWidth === 'content';
            const self    = grid && isContent  ? chipJustifySelf(s.align)    : undefined;
            const justify = grid && !isContent ? chipJustifyContent(s.align) : undefined;
          ...
          {entries.map((e, i) => {
            ...
            return (
              ...
              <span key={i} data-stat-key={e.key} className="inline-flex flex-col items-center text-center min-w-0">
                ...
                {e.label && <span className={cn('text-[10px] uppercase tracking-wider max-w-full text-muted-foreground break-words',
                  // KRO-222 — la etiqueta envuelve a 2 líneas por defecto (no se trunca a 1);
                  // break-words parte las palabras largas (p.ej. "DESCUBIERTO") en vez de
                  // cortarlas sin ellipsis; el publisher la recorta con appearance.truncate.
