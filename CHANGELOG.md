...
## [Unreleased]
### Added
- **`cardShapeBoxInside(fmt, box)`** (KRO-471): dice si una caja cabe entera dentro de la silueta de la carta, con la misma geometría que `cardShapeBadgeInset`. Lo usa el paquete de impresión de Studio para avisar de un QR que el troquel cortaría. Es un helper de datos: no cambia el contrato ni bumpea `protocolVersion`.
- **`cardShapeBadgeInset(fmt, corner, box)`** (KRO-232): cuánto meter una marca de la rejilla (número, «la tienes») desde su esquina para que quepa ENTERA dentro de la silueta de la carta. Mínimo, calculado sobre el path escalado, con tope `CARD_SHAPE_BADGE_INSET_MAX` (30 %); `null` si no hay silueta o no cabe. Studio y `core_dart` lo leen de aquí para colocar las marcas en el mismo sitio. DATA/aditivo: no bumpea el KRP.
- `CardRefHint` y el segundo argumento opcional de `CardRefResolver` (`@kromia/react`) — `MiniCardRefs` le pasa al resolutor la fracción del ancho del slot que ocupa cada mini-carta (`widthFraction`: 1/columnas en rejilla, ajustada por `refSize`; `refSize`% en carrusel). No son píxeles, porque el SDK no mide el DOM: el host la multiplica por el ancho que conoce de su marco y pide la foto a ese tamaño en vez del original (KRO-339). Aditivo: un resolutor de un solo argumento sigue valiendo. Render-only, no bumpea `protocolVersion`.
- `resolveCardTitleKey(fields, cardTitleKey?, primaryKey?)` — la **clave** del campo que titula la carta, con la misma prioridad que `resolveCardTitle`, que pasa a apoyarse en ella (un solo criterio, no dos que puedan separarse). Hacía falta para poder **ocultar ese campo del cuerpo** del detalle: quien pinta la ficha sabía el valor del título pero no cuál era la fila que ya estaba arriba, así que se leía «Ignis» en la cabecera y «Nombre: Ignis» otra vez debajo. Lo pidió el chat de Mobile al auditar su lado, y con razón: deducir la prioridad en la app habría sido la TERCERA copia del mismo criterio (TS, `core_dart`, la app), que es el drift exacto que `resolveCardTitle` vino a cerrar. Es **estructural** —no recibe la carta— porque si dependiera del valor, dos cartas de la misma sección ocultarían campos distintos y el detalle cambiaría de forma según cuál abras. NO bumpea el `protocolVersion` (KRO-223)
